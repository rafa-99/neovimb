...
{
	/**
	 * This implementation is quite 'brute force', same as in neovimb2
	 *  - both X clipboards are always set, PRIMARY and CLIPBOARD
	 *  - the X clipboards are always set, even though a neovimb register was given
	 */
	...
	/* store in neovimb default register */
	vb_register_add(c, '"', yanked);
	/* store in neovimb register buf if buf != 0 */
