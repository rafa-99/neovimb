 ...
 */
gboolean autocmd_run(Client *c, AuEvent event, const char *uri,
		const char *group) {
	GSList *lg, *lc;
	AuGroup *grp;
	AutoCmd *cmd;
...
gboolean autocmd_fill_group_completion(Client *c, GtkListStore *store,
		const char *input) {
	GSList *lg;
	...
	if (!input || !*input) {
		for (lg = c->autocmd.groups; lg; lg = lg->next) {
			...
			gtk_list_store_set(store, &iter, COMPLETION_STORE_FIRST,
					((AuGroup *)lg->data)->name, -1);
			found = true;
		}
	} else {
		for (lg = c->autocmd.groups; lg; lg = lg->next) {
...
gboolean autocmd_fill_event_completion(Client *c, GtkListStore *store,
		const char *input) {
	int i;
	const char *value;
	gboolean found = false;
	...
	if (!input || !*input) {
		for (i = 0; i < LENGTH(events); i++) {
			gtk_list_store_append(store, &iter);
			gtk_list_store_set(store, &iter, COMPLETION_STORE_FIRST, events[i].name,
					-1);
			found = true;
		}
	} else {
...
static guint get_event_bits(Client *c, const char *name) {
	...
	while (*name) {
		...
		for (i = 0; i < LENGTH(events); i++) {
			...
			len = 0;
			while (name[len] && events[i].name[len] &&
					name[len] == events[i].name[len]) {
				len++;
			}
