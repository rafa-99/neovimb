...
gboolean ext_dom_is_editable(WebKitDOMElement *element) {
	...
	if (WEBKIT_DOM_IS_HTML_INPUT_ELEMENT(element)) {
		type = webkit_dom_html_input_element_get_input_type(
		...
		if (!*type || !g_ascii_strcasecmp(type, "text") ||
				...
				!g_ascii_strcasecmp(type, "tel") || !g_ascii_strcasecmp(type, "time") ||
				!g_ascii_strcasecmp(type, "url") || !g_ascii_strcasecmp(type, "week")) {
			result = TRUE;
		}
...
gboolean ext_dom_focus_input(WebKitDOMDocument *doc) {
	...
	collection = webkit_dom_document_get_elements_by_tag_name_as_html_collection(
			doc, "html");
	if (!collection) {
		return FALSE;
	}
	...
	for (i = 0; i < len; i++) {
		...
		frame_doc = webkit_dom_html_iframe_element_get_content_document(
...
char *ext_dom_editable_get_value(WebKitDOMElement *element) {
	...
	if ((webkit_dom_html_element_get_is_content_editable(
					WEBKIT_DOM_HTML_ELEMENT(element)))) {
		value = webkit_dom_html_element_get_inner_text(
				WEBKIT_DOM_HTML_ELEMENT(element));
	} else if (WEBKIT_DOM_IS_HTML_INPUT_ELEMENT(
				WEBKIT_DOM_HTML_INPUT_ELEMENT(element))) {
		value = webkit_dom_html_input_element_get_value(
				WEBKIT_DOM_HTML_INPUT_ELEMENT(element));
	} else {
